...
def plotNWMStreamflowData(dischargedata, usgs_sites):
    ...
    for usgs_site in usgs_sites:
        ...
        plt.plot(data['Date'], data['Discharge'], label=f'USGS streamflow for gauge site: {usgs_site}', linewidth=2)
...
def plotUSGSStreamflow(huc, usgs_sites):
    code_dir, data_dir, output_dir = setup_directories()
    discharge_dir = os.path.join(output_dir, f"flood_{huc}", 'discharge', 'usgs_streamflow')
