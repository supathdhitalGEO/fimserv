...
def plotsrc(file, hydro_ids, branch_ids, discharge_value=None):
    ...
    for i, data in enumerate(data_list):
        plt.plot(
            data['default_discharge_cms'], data['stage'],
        ...
        if discharge_value:
            ...
            plt.plot([discharge_value, discharge_value], [0, stage_at_discharge], linestyle="--", color=colors[i], linewidth=2)
            plt.plot([0, discharge_value], [stage_at_discharge, stage_at_discharge], linestyle="--", color=colors[i], linewidth=2)
            # Marker at the intersection
            plt.scatter(
                discharge_value, stage_at_discharge,
                color=marker_fill_colors[i % len(marker_fill_colors)], edgecolor='black', s=200, marker='*', zorder=5,
